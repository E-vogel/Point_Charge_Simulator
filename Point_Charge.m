function Point_Charge()
    ...
    Ts{1} = text(ss{1}.XData - 0.038, ss{1}.YData, pulse, 'FontSize', 20, 'Color', 'w');
    Ts{2} = text(ss{2}.XData - 0.038, ss{2}.YData, minus, 'FontSize', 20, 'Color', 'w');
    ...
    i = 2;
    while ~all(switch_state == 0, 'all')
        Index = ones(n, 1);
        for k = 1:length(q)
            switch_state(X(:, i - 1, k) > 2.1, k) = 0;
            switch_state(Y(:, i - 1, k) > 2.1, k) = 0;
            switch_state(X(:, i - 1, k) < -0.1, k) = 0;
            switch_state(Y(:, i - 1, k) < -0.1, k) = 0;
            for ii = 1:n
                ...
                idx = sub2ind(size(x), y_idx, x_idx);
                Index(ii) = idx;
            end
            X(:, i, k) = X(:, i - 1, k) + sign(q(k)) * (EX(Index) ./ E(Index)) .* switch_state(:, k) * dl * 0.01;
            Y(:, i, k) = Y(:, i - 1, k) + sign(q(k)) * (EY(Index) ./ E(Index)) .* switch_state(:, k) * dl * 0.01;
        end
        i = i + 1;
    end
    ...
    for ii = 1:n
        for k = 1:length(q)
            ...
            if q(k) < 0 && (any(X(ii, :, k) < 0.1) || any(X(ii, :, k) > 1.9) || any(Y(ii, :, k) < 0.1) || any(Y(ii, :, k) > 1.9)) || q(k) > 0
        ...
        while ~all(switch_state == 0, 'all')
            ...
            for k = 1:length(q)
                ...
                for ii = 1:n
                    ...
                    y_idx = round(Y(ii, i - 1, k) / h);
                    g = 0;
                    if x_idx <= g || y_idx <= g || x_idx >= length(x(:, 1)) - g || y_idx >= length(y(:, 1)) - g
                        switch_state(ii, k) = 0;
                        continue
                    end
                    idx = sub2ind(size(x), y_idx, x_idx);
    ...
    function myBDCallback(src, ~)
        ...
        function myBMCallback(~, ~)
            ...
            if length(X(1, 1, :)) ~= length(q)
                X(n, N, end + 1) = NaN;
                Y(n, N, end + 1) = NaN;
            end
            for iii = 1:n
                for kk = 1:length(q)
                    handle{iii, kk}.XData = X(iii, 1:i - 1, kk);
                    handle{iii, kk}.YData = Y(iii, 1:i - 1, kk);
                    if q(kk) < 0 && (any(X(iii, :, kk) < 0.1) || any(X(iii, :, kk) > 1.9) || any(Y(iii, :, kk) < 0.1) || any(Y(iii, :, kk) > 1.9)) || q(kk) > 0
                        handle{iii, kk}.Visible = 'on';
                    else
                        handle{iii, kk}.Visible = 'off';
                    end
                end
            end
        end
